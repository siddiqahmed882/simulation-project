...
export default function Home() {
  ...
  function poissonPDF(x, lambda) {
    ...
    const factorialX =
      Math.exp(x * Math.log(x) - x - Math.log(2 * Math.PI)) * Math.sqrt(2 * Math.PI * x);
  ...
  const M_M_1_Table = async () => {
    ...
    for (let i = 0; i < cpLookupTable.length - 2; i++) {
      ...
      const findIndexInRange = (value, lowerBoundArray, upperBoundArray) => {
        ...
        console.log('upperBoundArray[i]', upperBoundArray[i].cumulativeProbability);
    ...
    const Lq =
      Math.pow(parseFloat(utilizationFactor), 2) / (1 - parseFloat(utilizationFactor));
  ...
  const M_M_C_Table = async () => {
    ...
    function mmcSchedule(arrivalTimes, serviceTimes, numServers) {
      ...
      for (let i = 0; i < cpLookupTable.length - 1; i++) {
        ...
        const startTime = Math.max(arrivalTime, serverBusyTimes[availableServer]);
        ...
        turnaround_Time.push(turnaroundTime > 0 ? turnaroundTime : -turnaroundTime);
    ...
    const metrics = mmcMetrics(parseFloat(arrivalRate), parseFloat(serviceRate), servers);
  ...
  const M_G_1_Table = async () => {
    ...
    function mgcMetrics(arrivalRate, serviceRate, serviceMin, serviceMax, numServers) {
      if (
        arrivalRate <= 0 ||
      ...
      const Lq =
        ((Math.pow(arrivalRate, 2) * Math.pow(Variance, 2) + Math.pow(rho, 2)) / 2) *
        (1 - rho);
            ...
            <tbody>
              {cpValues.map((value, index) => (
                <tr key={index}>
                  ...
                  <td className='  px-4'>{value.cumulativeProbability.toFixed(6)}</td>
                  <td className='  px-4'>{value.x}</td>
                  <td className='  px-4'>{interArrivalTimesRand[index].toFixed(4)}</td>
                  ...
                  <td className={`px-4 ${startTimes[index] < 0 ? 'text-red-500' : ''}`}>
                    {startTimes[index] || 0}
                  </td>
                  <td
                    className={`px-4 ${
                  ...
                  <td
                    className={`px-4 ${TurnaroundTime[index] < 0 ? 'text-red-500' : ''}`}
                  >
                    {TurnaroundTime[index] || 0}
                  </td>
                  <td className={`px-4 ${WaitTime[index] < 0 ? 'text-red-500' : ''}`}>
                    {WaitTime[index] || 1}
                  </td>
                  <td className={`px-4 ${ResponseTime[index] < 0 ? 'text-red-500' : ''}`}>
...
function QueueLengthViz({ customers }) {
  ...
  const calculateYPosition = (data, index, yScale) => {
    ...
    return Math.max(yScale(previousEnd) + overlappingThreshold, yScale(currentStart));
  };
